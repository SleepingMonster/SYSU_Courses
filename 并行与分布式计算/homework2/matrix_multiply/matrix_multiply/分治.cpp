...
typedef struct matrix {
	...
	static matrix common_matrix_product(matrix &A, matrix &B, int ar, int ac, int br, int bc, int n)
	{
		matrix c(n, n);
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < n; j++)
			{
				for (int k = 0; k < n; k++)
				{
					c.data[i][j] += A.data[ac + i][ar + k] * B.data[bc + k][br + j];
				}
			}
		}
		return c;
...
int main()
{
	...
	//matrix D(matrix::common_matrix_product(A, B, 0, 0, 0, 0, dim));
	//clock_t end2 = clock();
	cout << "分治算法:" << (double)(end1 - start1) / CLOCKS_PER_SEC << "s" << endl;
	//cout << "一般算法:" << (double)(end2 - start2) / CLOCKS_PER_SEC << "s" << endl;
	return 0;
}